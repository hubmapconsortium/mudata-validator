...
def validate_anndata(input_data):
    ...
    unused_obs_columns = [col for col in adata.obs.columns if col not in ['original_obs_id', 'object_type']]  # Columns that are not used in validation
    unused_obsm_keys = [key for key in adata.obsm.keys() if key not in ['X_spatial', 'X_embedding']]  # Keys not directly used in validation
